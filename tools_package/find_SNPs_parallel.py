...
def main_run(results_df_path, num_processes=8):
    ...
    pool.map(run_script, [f"{results_df_path_root}_{slice}.csv" for slice in range(num_processes)])
    pool.close()
    pool.join()
...
main_run(results_df_path, num_processes=num_processes)
